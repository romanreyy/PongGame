...
public class fondo extends JPanel implements ActionListener, KeyListener {
    ...
    private double ballXSpeed = 4, ballYSpeed = 4;
    ...
    private double initialSpeed = 3.0; // Velocidad inicial de saque
    private double afterFirstTouchSpeed = 5.0; // Velocidad después del primer toque
    ...
    private final double SPEED_INCREASE = 0.2; // 20% de aumento
    ...
    public fondo() {
        ...
        restartButton.setBounds(GAME_WIDTH / 2 - 150, GAME_HEIGHT / 2 + 20, 120, 40);
        exitButton.setBounds(GAME_WIDTH / 2 + 30, GAME_HEIGHT / 2 + 20, 120, 40);

        restartButton.addActionListener(e -> restartGame());
        exitButton.addActionListener(e -> System.exit(0));

        // Establecer un layout nulo para permitir el uso de setBounds
        setLayout(null);
    ...
    private void updateGameTime() {
        ...
        if (gameTime >= HALF_TIME_DURATION) {
            if (!isSecondHalf) {
                ...
                // Mostrar los botones "Reiniciar" y "Salir" al final del partido
                restartButton.setVisible(true);
                exitButton.setVisible(true);

                // Forzar repintado y revalidación
                this.revalidate();
                this.repaint();
            }
        }
    ...
    private void switchSides() {
        ...
        double savedSpeed = Math.sqrt(ballXSpeed * ballXSpeed + ballYSpeed * ballYSpeed);
    ...
    private void resetGame() {
        ...
        ballXSpeed = initialSpeed * Math.cos(angle) * (Math.random() < 0.5 ? 1 : -1);
        ballYSpeed = initialSpeed * Math.sin(angle);
        
        waitingAfterGoal = true;
        gameTime = 0;
        firstImpact = true; // Reiniciamos firstImpact para que el próximo golpe use la velocidad después del primer toque
    }
    ...
    private void handlePaddleHit() {
        if (firstImpact) {
            // Cambiar la velocidad a afterFirstTouchSpeed después del primer impacto
            double angle = Math.atan2(ballYSpeed, ballXSpeed);
            ballXSpeed = afterFirstTouchSpeed * Math.cos(angle);
            ballYSpeed = afterFirstTouchSpeed * Math.sin(angle);
            firstImpact = false;
        } else {
            touchCount++;
            increaseSpeed();
